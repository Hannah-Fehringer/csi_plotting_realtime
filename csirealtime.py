...
class GetDataThread(threading.Thread):
    def __init__(self, device):
        ...
        if device == 'intel':
            self.csidata = csiread.Intel(None, 3, 2)
        if device == 'nexmon':
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            while self.running:
                ...
                if code == 0xbb:    # intel
                    ...
                    mutex.acquire()
                    cache_data1.pop(0)
                    cache_data1.append(scaled_csi_sm[25, 0, 0])
                    cache_data2.pop(0)
                    cache_data2.append(scaled_csi_sm[25, 1, 0])
                    cache_data3.pop(0)
                    cache_data3.append(scaled_csi_sm[25, 2, 0])
                    mutex.release()
                    count += 1
...
def realtime_plot_intel(task):
    ...
    ax.set_ylim(0, 60)
    ax.set_xlim(800, cache_len)
    x = np.arange(0, cache_len, 1)

    line1,  = ax.plot(x, np.abs(cache_data1), linewidth=1.0, label='subcarrier_25_0_0')
    line2,  = ax.plot(x, np.abs(cache_data2), linewidth=1.0, label='subcarrier_25_1_0')
    line3,  = ax.plot(x, np.abs(cache_data3), linewidth=1.0, label='subcarrier_25_2_0')
    plt.legend()
